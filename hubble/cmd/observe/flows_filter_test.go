...
func TestConflicts_Table(t *testing.T) {
	...
	conflicts := [][]string{
		...
		{"--from-pod", "--fqdn"},
		{"--from-pod", "--namespace"},
		{"--from-pod", "--all-namespaces"},
		{"--to-pod", "--fqdn"},
		{"--to-pod", "--to-fqdn"},
		{"--to-pod", "--namespace"},
		{"--to-pod", "--all-namespaces"},
...
func TestNamespace(t *testing.T) {
	...
	tt := []struct {
		...
		},
		{
			...
			filters: []*flowpb.FlowFilter{},
			err:     `conflicting namespace: "kube-system" does not contain "cilium/foo-9c76d6c95-tf788"`,
		},
		{
			name:    "conflicting svc and namespace",
			flags:   []string{"--from-namespace", "kube-system", "--from-service", "cilium/foo"},
			filters: []*flowpb.FlowFilter{},
			err:     `conflicting namespace: "kube-system" does not contain "cilium/foo"`,
		},
		{
			name:    "conflicting svc and pod",
			flags:   []string{"--from-service", "cilium/foo", "--from-pod", "kube-system/hubble"},
			filters: []*flowpb.FlowFilter{},
			err:     `conflicting namespace: namespace of service "cilium/foo" conflict with pod "kube-system/hubble"`,
		},
		{
